...
router.post('/register',(req,res)=>{
  ...
  pool.query('INSERT INTO huali_user SET ?',[obj],(err,result)=>{
    if(err) throw err;
    //是否添加成功
...
router.post('/login',(req,res)=>{
  ...
  pool.query('SELECT * FROM huali_user WHERE uname=? AND upwd=?',[$uname,$upwd],(err,result)=>{
    if(err) throw err;
    //判断查询的结果（数组）长度是否大于0
...
router.get('/checkuname',(req,res)=>{
  ...
  pool.query('SELECT uid FROM huali_user WHERE uname=?',$uname,(err,result)=>{
    if(err) throw err;
    if(result.length>0){
